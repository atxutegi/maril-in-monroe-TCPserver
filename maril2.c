...
void handle_connection(int thread_num, MY_SOCK sock)
{
    ...
    for (;;)
    {
	    ...
	    syslog_and_print(fp, LOG_INFO,"ALMOST FINISHED");
            return;
        }
        /***** PING *****/
    ...
    syslog_and_print(fp, LOG_INFO,"FINISHED");
    fclose(fp);
    pid_t pid2;
    if(thread_num==0)
        {
		char *commandKill="sudo sh testKiller.sh";
                pid2=fork();
...
static void *worker_thread_main(void *arg)
{
	...
	FILE *fp2;
    	fp2=fopen("outputLog.txt", "a");
	syslog_and_print(fp2, LOG_INFO,"READY TO LAUNCH PREV"); //NQAS
	...
	if(thread_num==0)
	{
		...
		char *dir=buf;
		char buffering[128];
		strncpy(buffering, buf, sizeof(buf));
		syslog_and_print(fp2, LOG_INFO, "BEFORE BEFORE");
		char* test;
		test = strtok(buffering, "ffff");
		test = strtok (NULL, ":");
		test = strtok (NULL, ":");
		syslog_and_print(fp2, LOG_INFO, "TRIAL %s\n", test);
		char *nameSS;
		char *nameTCP;
		char *commandSS="sudo sh ssTrace.sh ";
		char *commandTCP1="sudo tcpdump -i eth0 'host ";
		char *commandTCP2="' -s 0 -n -w ";
		char *commandTCP3=".pcap";
		name_merged=malloc(strlen(buffer2)+strlen(dir)+1+1);
		strcpy(name_merged, buffer2);
		strcat(name_merged, "-");
		strcat(name_merged, test);
		syslog_and_print(fp2, LOG_INFO,name_merged); //NQAS
		nameSS=malloc(strlen(commandSS)+strlen(test)+strlen(name_merged)+1+1);
		strcpy(nameSS, commandSS);
		strcat(nameSS, test);
		strcat(nameSS, " ");
		strcat(nameSS, name_merged);
		nameTCP=malloc(strlen(commandTCP1)+strlen(commandTCP2)+strlen(name_merged)+strlen(commandTCP3)+strlen(test)+1+1);
		strcpy(nameTCP, commandTCP1);
		strcat(nameTCP, test);
		strcat(nameTCP, commandTCP2);
		strcat(nameTCP, name_merged);
		strcat(nameTCP, commandTCP3);
				
		
		if (pid==0) { /* child process */
			system(nameSS);
			syslog_and_print(fp2, LOG_INFO,"LAUNCHED"); //NQAS
    		}
		pid_2=fork();
                syslog_and_print(fp2, LOG_INFO,"BEFORE PID_2"); //NQAS
                if (pid_2==0) { /* child process */
			system(nameTCP);
                        syslog_and_print(fp2, LOG_INFO,"LAUNCHED_2"); //NQAS
		}
	}
	...
	//if(thread_num==0)
        //{
        //        syslog(LOG_ERR, "READY TO KILL"); //NQAS EHU
        //        pid_t pid2=fork();
        //        if (pid2==0) { /* child process */
        //                system("sudo sh /home/operario/maril-in-monroe-TCPserver/testKiller.sh test");
        //                syslog(LOG_ERR, "KILLED");
        //        }
        //}
