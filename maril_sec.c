...
static void *worker_thread_main(void *arg)
{
	...
	if(thread_num==0)
	{
		...
		char buffer [126];
		syslog_and_print(fp2, LOG_INFO,"BEFORE PID"); //NQAS
		clock_t clock_ss = clock();
		sprintf(buffer,"%ld", clock_ss);
		char *buffer2=buffer;
		char *dir=buf;
		char* name_merged;
		...
		strcat(name_merged, "-");
		strcat(name_merged, dir);
		syslog_and_print(fp2, LOG_INFO,name_merged); //NQAS		
		
		
		if (pid==0) { /* child process */
        		//char *argv[]={buf,naming,NULL};
			//system("sudo sh /home/operario/maril-in-monroe-TCPserver/ssTrace.sh 85.85.15.100 test");
			system("sudo sh /home/operario/maril-in-monroe-TCPserver/ssTrace.sh 158.227.56.212 test");
		...
		pid_2=fork();
                ...
                if (pid_2==0) { /* child process */
                        //char *argv[]={buf,naming,NULL};
                        //system("sudo tcpdump -i eth0 'host 85.85.15.100' -s 0 -n -w test.pcap");
			system("sudo tcpdump -i eth0 'host 158.227.56.212' -s 0 -n -w test.pcap");
